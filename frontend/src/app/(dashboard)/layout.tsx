    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'ldc_clinic_store',
        },
        (payload) => {
          const newRecord = payload.new as any;
          if (newRecord && newRecord.id && newRecord.data) {
            // Update local storage silently
            localStorage.setItem(newRecord.id, JSON.stringify(newRecord.data));
            // Dispatch event to trigger state reload across all active pages
