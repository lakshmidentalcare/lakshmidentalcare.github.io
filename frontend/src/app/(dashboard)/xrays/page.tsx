...
export default function XRaysAIPage() {
  ...
  const handleDeleteXRay = async (id: string) => {
    if (confirm('Are you sure you want to permanently delete this X-Ray record?')) {
      const updated = xraysList.filter(x => x.id !== id);
      await saveXRaysToStorage(updated);
      setSelectedXRay(updated.length > 0 ? updated[0] : null);
    }
  };
      ...
      <div className="bg-gradient-to-r from-[#1E0726] via-[#32113E] to-[#5D2471] text-white p-8 rounded-3xl border border-[#73308A]/40 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <button 
              onClick={() => handleDeleteXRay(selectedXRay.id)}
              className="bg-white hover:bg-rose-50 text-slate-700 hover:text-rose-600 border border-[#73308A]/15 hover:border-rose-200 font-semibold px-4 py-3 rounded-2xl flex items-center space-x-2 shadow-xs transition-all"
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <SearchableSelect 
                  ...
                  onChange={(val) => setSelectedXRay(xraysList.find(x => x.id === val) || xraysList[0])}
                  className="font-bold text-slate-800 bg-transparent text-base outline-none cursor-pointer focus:text-[#73308A]"
                  options={xraysList.map(x => ({ value: x.id, label: `${x.patientName} (${x.patientCode}) — ${x.type}` }))}
                />
              </div>
            </div>
            ...
            <form onSubmit={handleSaveUpload} className="p-6 space-y-4 text-xs">
              <div>
                ...
                {patients.length > 0 ? (
                  <SearchableSelect 
                    ...
                    options={[
                      ...patients.map(p => ({ value: p.id, label: `${p.name} (${p.patientCode})` })),
                      { value: "NEW", label: "➕ Walk-in / Unregistered Patient" }
                    ]}
                  />
                ) : (
                  <input 
              ...
              <div>
                ...
                <SearchableSelect 
                  ...
                  options={[
                    { value: "Panoramic OPG (Full Mouth)", label: "Panoramic OPG (Full Mouth)" },
                    { value: "Bitewing (Premolar/Molar)", label: "Bitewing (Premolar/Molar)" },
                    { value: "IOPA Digital Radiograph", label: "IOPA Digital Radiograph" },
                    { value: "CBCT 3D Scan", label: "CBCT 3D Scan" },
                    { value: "Intraoral Photo", label: "Intraoral Photo" }
                  ]}
                />
              </div>
